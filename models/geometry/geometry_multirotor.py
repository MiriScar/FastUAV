...
class Geometry(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem("landing_gear", LandingGearGeometry(), promotes=["*"])
...
class LandingGearGeometry(om.ExplicitComponent):
    """
    Computes landing gear geometry
    """
    def initialize(self):
        self.options.declare("propulsion_id", default=MR_PROPULSION, values=[MR_PROPULSION])

    def setup(self):
        self.add_input("data:geometry:landing_gear:legs:clearance", units="m")
        self.add_input("data:geometry:landing_gear:legs:alpha", units="rad")
        self.add_input("data:structures:landing_gear:k", units=None)

        self.add_output("data:geometry:landing_gear:legs:length", units="m")
        self.add_output("data:geometry:landing_gear:feet:length", units="m")
    ...
    def compute(self, inputs, outputs):
        clearance = inputs["data:geometry:landing_gear:legs:clearance"]
        alpha = inputs["data:geometry:landing_gear:legs:alpha"]
        foot_leg_ratio = inputs["data:structures:landing_gear:k"] 

        L_legs = clearance / (np.cos(alpha))  # [m] length of the legs 
        Lfeet = foot_leg_ratio*L_legs         # [m] length of the feet

        outputs["data:geometry:landing_gear:legs:length"] = L_legs
        outputs["data:geometry:landing_gear:feet:length"] = Lfeet