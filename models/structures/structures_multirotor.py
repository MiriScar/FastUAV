...
class Structures(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem("landing_gear", LandingGearWeight(), promotes=["*"])
...
class LandingGearWeight(om.ExplicitComponent):
    ...
    def setup(self):
        self.add_input("data:geometry:landing_gear:legs:number", val=np.nan)
        self.add_input("data:geometry:landing_gear:legs:length", val=np.nan, units="m")
        self.add_input("data:geometry:landing_gear:feet:length", val=np.nan, units="m")
        self.add_input("data:weight:landing_gear:density", val=np.nan, units="kg/m**3")
        self.add_input("data:structures:arms:diameter:outer", val=np.nan, units="m")
        self.add_input("data:structures:arms:diameter:inner", val=np.nan, units="m")

        self.add_output("data:geometry:landing_gear:cross_section_area", units="m**2", lower=0.0)
        self.add_output("data:weight:airframe:landing_gear:mass", units="kg")
    ...
    def compute(self, inputs, outputs):     
        Nlegs = inputs["data:geometry:landing_gear:legs:number"]
        L_legs = inputs["data:geometry:landing_gear:legs:length"]
        Lfeet = inputs["data:geometry:landing_gear:feet:length"]
        rho = inputs["data:weight:landing_gear:density"]
        Dout = inputs["data:structures:arms:diameter:outer"] 
        Din = inputs["data:structures:arms:diameter:inner"]



        # Landing gear cross section area calculation
        S = (np.pi/4*(Dout**2 - Din**2)) # [m**2] area of the tubular cross section

        # Landing gear mass calculation (legs + feet)
        Mlanding_gear = (
           np.pi * (Dout**2 - Din**2) * L_legs * rho * Nlegs +
           np.pi * (Dout**2 - Din**2) * Lfeet * rho * 2
        )   # [kg] mass of the landing gear

        outputs["data:geometry:landing_gear:cross_section_area"] = S
        outputs["data:weight:airframe:landing_gear:mass"] = Mlanding_gear
